...
# Various coordinate system transformations and related utilities for GNSS data processing that may or may not be needed.
# No docstrings provided since these are mostly standard formulas that can be easily looked up, and the code should be self-explanatory
...
def ecef_to_lla_torch(ecef: torch.Tensor) -> torch.Tensor:
    x, y, z = ecef
...
def lla_to_ecef_torch(lla: torch.Tensor) -> torch.Tensor:
    lat, lon, alt = lla
...
def ecef_to_neu_rot(ref_ecef: np.ndarray) -> np.ndarray:
...
def ecef_to_ned_rot(ref_ecef: np.ndarray) -> np.ndarray:
    R = ecef_to_neu_rot(ref_ecef).copy()
    R[2, :] *= -1.0
    return R


def ecef_to_enu_rot(ref_ecef: np.ndarray) -> np.ndarray:
    ref_lla = ecef_to_lla(ref_ecef)
    lat, lon = map(np.deg2rad, ref_lla[:2])
...
def ecef_to_end_rot(ref_ecef: np.ndarray) -> np.ndarray:
    R = ecef_to_enu_rot(ref_ecef).copy()
    R[2, :] *= -1.0
    return R

def neu_to_ecef_rot(ref_ecef: np.ndarray) -> np.ndarray:
    return ecef_to_neu_rot(ref_ecef).T

def ned_to_ecef_rot(ref_ecef: np.ndarray) -> np.ndarray:
    return ecef_to_ned_rot(ref_ecef).T

def enu_to_ecef_rot(ref_ecef: np.ndarray) -> np.ndarray:
    return ecef_to_enu_rot(ref_ecef).T

def end_to_ecef_rot(ref_ecef: np.ndarray) -> np.ndarray:
    return ecef_to_end_rot(ref_ecef).T

def ecef_to_ned(ecef: np.ndarray, ref_ecef: np.ndarray) -> np.ndarray:
    R = ecef_to_ned_rot(ref_ecef)
    return R @ (ecef - ref_ecef)

def ned_to_ecef(ned: np.ndarray, ref_ecef: np.ndarray) -> np.ndarray:
    R = ned_to_ecef_rot(ref_ecef)
    return R @ ned + ref_ecef

def ecef_to_neu(ecef: np.ndarray, ref_ecef: np.ndarray) -> np.ndarray:
    R = ecef_to_neu_rot(ref_ecef)
    return R @ (ecef - ref_ecef)

def neu_to_ecef(neu: np.ndarray, ref_ecef: np.ndarray) -> np.ndarray:
    R = neu_to_ecef_rot(ref_ecef)
    return R @ neu + ref_ecef

def heading_speed_to_ecef(heading_deg: float, speed_m_s: float, lat_deg: float, lon_deg: float) -> np.ndarray:
...
def errors_haversine(est_lla: np.ndarray, truth_lla: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
